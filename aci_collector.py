...
class Apic():
    ...
    def aci_get_fabric_usage(self):
        ...
        try:
            ...
            if not response['imdata']:
                ...
                docs.append({'fisaci.fabric_name': self.fabric_name,
                            'fisbeat.doc_type': 'aci_fabric_capacity',
                            'fisaci.tn_max': tn_max,
                            'fisaci.epg_max': epg_max,
                            'fisaci.vrf_max': vrf_max,
                            'fisaci.contracts_max': contracts_max,
                            'fisaci.bd_max': bd_max})
            ...
            docs.append({'fisaci.fabric_name': self.fabric_name,
                         'fisbeat.doc_type': 'aci_fabric_usage',
                         'fisaci.tn_count': tn_count,
                         'fisaci.vrf_count': vrf_count,
                         'fisaci.contracts_count': contracts_count,
                         'fisaci.contact_filters_count': contract_filters_count,
                         'fisaci.bd_count': bd_count,
                         'fisaci.epg_count': epg_count})
            ...
            for tn, epg_count in epgs_per_tenant.items():
                docs.append({'fisaci.fabric_name': self.fabric_name,
                             'fisbeat.doc_type': 'aci_epgs_per_tenant',
                             'fisaci.tenant.name': tn,
                             'fisaci.tenant.epg_count': epg_count})
        ...
        try:
            ...
            for k,v in node_eqpt_capacity.items():
                if k in node_fwdprof_limits and k in node_ctx_status: 
                    node_capacity_result.append({'fisaci.dn': k,
                                                    'fisaci.fabric_name': self.fabric_name,
                                                    'fisaci.hlq': self.fabric_name + '/' + k,
                                                    'fisbeat.doc_type': 'aci_node_limits',
                                                    'fisaci.leaf_lpm_usage': v['LPM'],
                                                    'fisaci.leaf_policy_usage': v['PolCAM'],
                                                    'fisaci.leaf_polOTcam_usage': v['PolOTCAM'],
                                                    'fisaci.leaf_bd_limit': int(node_fwdprof_limits[k]['bd']),
                                                    'fisaci.leaf_epIpv4_limit': int(node_fwdprof_limits[k]['epIpv4']),
                                                    'fisaci.leaf_epMac_limit': int(node_fwdprof_limits[k]['epMac']),
                                                    'fisaci.leaf_epg_limit': int(node_fwdprof_limits[k]['epg']),
                                                    'fisaci.leaf_lpm_limit': int(node_fwdprof_limits[k]['lpm']),
                                                    'fisaci.leaf_policy_limit': int(node_fwdprof_limits[k]['policy']),
                                                    'fisaci.leaf_slash32_limit': int(node_fwdprof_limits[k]['slash32']),
                                                    'fisaci.leaf_vlan_limit': int(node_fwdprof_limits[k]['vlan']),
                                                    'fisaci.leaf_vrf_limit': int(node_fwdprof_limits[k]['vrf']),
                                                    'fisaci.leaf_bd_usage': int(node_ctx_status[k]['l2BD']),
                                                    'fisaci.leaf_epg_usage': int(node_ctx_status[k]['fvEpP']),
                                                    'fisaci.leaf_vrf_usage': int(node_ctx_status[k]['l3Dom'])})
            ...
            if rules_count:
                for hlq, contracts in rules_count.items():
                    for contract,count in contracts.items():
                        docs.append({'fisaci.fabric_name': self.fabric_name,
                                     'fisaci.hlq': hlq,
                                     'fisbeat.doc_type': 'aci_leaf_contract_rules',
                                     'fisaci.leaf_contract': contract,
                                     'fisaci.leaf_contract_rule_count': count
                                    })
            return {'rc': 0, 'data': docs}
                ...
                if response['imdata']:
                    if len(response['imdata']) == 1:
                        if response['imdata'][0].get('procSysMemHist5min', []):
                            ...
                            elastic_docs.append({'fisaci.fabric_name': self.fabric_name,
                                                'fisaci.hlq': self.fabric_name + '/' + node_dn,
                                                'fisbeat.doc_type': 'aci_leaf_mem_util',
                                                'fisaci.leaf_mem_used': mem_used,
                                                'fisaci.leaf_mem_total': mem_total,
                                                'fisaci.leaf_mem_used_percent': mem_used_percent})
            ...
            if response['imdata']:
                for item in response['imdata']:
                    ...
                    if node_dn in self.fabric_inventory:
                        ...
                        elastic_docs.append({'fisaci.fabric_name': self.fabric_name,
                                             'fisaci.hlq': self.fabric_name + '/' + node_dn,
                                             'fisbeat.doc_type': 'aci_leaf_ssd_warning',
                                             'fisaci.ssd_warning': warning})
                ...
                for item in inv_list:
                    ...
                    if item_dict['role'] == 'leaf':
                        ...
                        docs.append({'host.name': inv_dict['device'],
                                     'fisbeat.doc_type': 'aci_leaf_inventory',
                                     'fisaci.hlq': self.fabric_name + '/' + inv_dict['dn'],
                                     'fisaci.dn': inv_dict['dn'],
                                     'fisaci.fabric_name': self.fabric_name,
                ...
                if item.get('rpmEntity'):
                    ...
                    if node_dn in self.fabric_inventory:
                        docs.append({'host.name': self.fabric_inventory[node_dn]['device'],
                                     'fisbeat.doc_type': 'aci_leaf_rpm',
                                     'fisaci.mo': 'rpmEntity',
                                     'fisaci.hlq': self.fabric_name + '/' + node_dn,
                                     'fisaci.pod': pod,
                                     'fisaci.node': node,
                                     'fisaci.fabric_name': self.fabric_name,
                                     'fisaci.shMemAllocFailCount': int(item['rpmEntity']['attributes'].get('shMemAllocFailCount', 0)),
                                     'fisaci.shMemTotal': int(item['rpmEntity']['attributes'].get('shMemTotal', 0)),
                                     'fisaci.shMemUsage': int(item['rpmEntity']['attributes'].get('shMemUsage', 0)),
                                     'fisaci.shMemAlert': item['rpmEntity']['attributes'].get('shMemAlert', '')
                                    })
        ...
        try:
            ...
            for node_dn, epg_count in epg_counts.items():
                if self.fabric_inventory.get(node_dn):
                    ...
                    if node_dn in self.fabric_inventory:
                        docs.append({'fisbeat.doc_type': 'aci_extepg_per_leaf',
    ...
    def aci_get_l3outs(self):
        ...
        try:
            epg_counts = {}
            count_field = 'fisaci.'+ 'l3extInstP' + '_count'
            ...
            for node_dn, epg_count in epg_counts.items():
                if node_dn in self.fabric_inventory:
                    pod = node_dn.split('/')[1].replace('pod-', '')
                    node = node_dn.split('/')[2].replace('node-', '')
                    docs.append({'fisbeat.doc_type': 'aci_extepg_per_l3out',
                                'host.name': self.fabric_inventory[node_dn]['device'],
                                'fisaci.mo': 'l3extInstP',
                                'fisaci.hlq': self.fabric_name + '/' + node_dn,
                                'fisaci.pod': pod,
                                'fisaci.node': node,
                                'fisaci.fabric_name': self.fabric_name,
                    ...
                    if response.status_code == 200:
                        ...
                        if response_data['imdata']:
                            ...
                            docs.append({'host.name': device['device'],
                                         'fisaci.dn': dn,
                                         'fisbeat.doc_type': 'aci_port_capacity',
                                         'fisaci.hlq': self.fabric_name + '/' + dn,
                                         'fisaci.fabric_name': self.fabric_name,
                                         'fisaci.ports_total': ports_total,
                                         'fisaci.ports_free': ports_free,
                                         'fisaci.ports_disabled': ports_disabled,
                                         'fisaci.ports_down': ports_down,
                                         'fisaci.ports_util_percent': round(ports_util_percent, ndigits=1),
                                         'fisaci.ports_up': ports_up})
                if docs:
                    return {'rc': 0, 'data': docs }
    ...
    def count_by_pod_node(self, mo):
        ...
        try:
            ...
            for node_dn, data_count in data_counts.items():
                ...
                if node_dn in self.fabric_inventory:
                    docs.append({'host.name': self.fabric_inventory[node_dn]['device'],
                                'fisbeat.doc_type': 'aci_mo_count_by_pod_node',
                                'fisaci.mo': mo,
                                'fisaci.hlq': self.fabric_name + '/' + node_dn,
                                'fisaci.pod': pod,
                                'fisaci.node': node,
